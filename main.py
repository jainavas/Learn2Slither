...
def train(agent, board, interpreter, display, num_episodes=10,
          verbose=True, show_visual=False, save_path=None):
    """
    ...
    for episode in range(num_episodes):
        ...
        while not game_over:
            ...
            estado = siguiente_estado if siguiente_estado is not None else \
                interpreter.get_compressed_state()
...
def test(agent, num_episodes=3, verbose=True, show_visual=False, delay_ms=200):
    ...
    print(f"\n=== RESULTADOS TESTING ===")
    print(f"Promedio - Length: {avg_length:.1f} | "
...
def main():
    parser = argparse.ArgumentParser(
        description='Learn2Slither - Snake con Q-learning'
    )
    ...
    parser.add_argument('-visual', type=str, default='off',
                        choices=['on', 'off'],
    ...
    parser.add_argument('-verbose', type=str, default='on',
                        choices=['on', 'off'],
                        help='Mostrar información detallada')
    parser.add_argument('-speed', type=int, default=200,
                        help='Velocidad de visualización en ms (default: 200, más bajo = más rápido)')
    ...
    if args.dontlearn:
        ...
        test(agent, num_episodes=args.sessions, verbose=verbose,
             show_visual=show_visual, delay_ms=args.speed)
    else:
        # Modo entrenamiento
        print("=== TRAINING MODE ===")
        board = Board()
        ...
        train(agent, board, interpreter, display,
