...
if not TKINTER_AVAILABLE and not PYGAME_AVAILABLE:
    print("Warning: No GUI library (tkinter/pygame). Visual mode disabled.")
...
class Display:
    ...
    def _init_tkinter(self):
        ...
        self.canvas = tk.Canvas(
            self.window, width=canvas_width, height=canvas_height, bg="black"
        )
    ...
    def _draw_tkinter(self, board_obj, episode, steps):
        ...
        for i in range(self.board_size + 1):
            ...
            self.canvas.create_line(
                x, 0, x, self.board_size * self.cell_size, fill="gray30"
            )
            # Líneas horizontales
            y = i * self.cell_size
            self.canvas.create_line(
                0, y, self.board_size * self.cell_size, y, fill="gray30"
            )
        ...
        y2 = y1 + self.cell_size - 4
        self.canvas.create_rectangle(
            x1, y1, x2, y2, fill="cyan", outline="white", width=2
        )
    ...
    def _init_pygame(self):
        ...
        window_height = self.board_size * self.cell_size + 60  # +60 para info
    ...
    def _draw_pygame(self, board_obj, episode, steps):
        ...
        pygame.draw.rect(self.screen, (255, 255, 255), head_rect, 2)  # Blanco
